...
class DataModuleKmers(pl.LightningDataModule):
    def __init__(self, batch_size, k, load_data=False ): 
        super().__init__() 
        self.batch_size = batch_size 
        # hard code to avoid recomputing each time: 
        vocab2idx = {'<start>': 0, '<stop>': 1, '-': 2, 'A': 3, 'C': 4, 'D': 5, 'E': 6, 'F': 7, 'G': 8, 'H': 9, 'I': 10, 'K': 11, 'L': 12, 'M': 13, 'N': 14, 'P': 15, 'Q': 16, 'R': 17, 'S': 18, 'T': 19, 'V': 20, 'W': 21, 'X': 22, 'Y': 23}
        vocab = ['<start>', '<stop>', '-', 'A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'X', 'Y']
        self.train  = DatasetKmers(dataset='train', k=k, vocab=vocab, vocab2idx=vocab2idx, load_data=load_data) 
        self.val    = DatasetKmers(dataset='val', k=k, vocab=self.train.vocab, vocab2idx=self.train.vocab2idx, load_data=load_data )
        self.test   = DatasetKmers(dataset='test', k=k, vocab=self.train.vocab, vocab2idx=self.train.vocab2idx, load_data=load_data )
...
class DatasetKmers(Dataset): # asssuming train data 
    def __init__(self, dataset='train', data_path=None, k=1, vocab=None, vocab2idx=None, load_data=False):
        ...
        if vocab is None or load_data:
            if data_path is None: 
                ...
                path_to_data = '../uniref_vae/gvp/seq_and_avg_encoding.pkl' 
            data_dict = pickle.load(open(path_to_data, 'rb'))
            # data dict is list of tuples (seq, avg_encoding), split into two lists
            seqs, avg_encodings = zip(*data_dict)
            train_seqs = seqs# df['sequence'].values  # 1_500_000  sequences
            training_encodings = avg_encodings # shape (512,) each
        ...
        self.encodings = []
        if load_data:
            for seq in tqdm(regular_data, total=len(regular_data)):
                token_num = 0
                kmer_tokens = []
                ...
                self.encodings.append(training_encodings[0])
        ...
        if self.dataset == 'train': # 90 %
            ...
            self.encodings = self.encodings[0:-ten_percent]
        elif self.dataset == 'val': # 5 %
            self.data = self.data[-ten_percent:-five_percent] 
            self.encodings = self.encodings[-ten_percent:-five_percent]
        elif self.dataset == 'test': # 5 %
            self.data = self.data[-five_percent:] 
            self.encodings = self.encodings[-five_percent:]
        else: 
...
def collate_fn(data):
    aas = [x[0] for x in data]
    # Length of longest molecule in batch 
    max_size = max([x.shape[-1] for x in aas])
    return (torch.vstack(
        # Pad with stop token
        [F.pad(x[0], (0, max_size - x[0].shape[-1]), value=1) for x in data]
    ), torch.tensor(np.array([x[1] for x in data])))
